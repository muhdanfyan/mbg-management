...
async function scrapeSppg() {
    const pageUrl = "https://sites.google.com/view/wadah-merah-putih/sppg";
        ...
        if ((url.includes('script.google.com') || url.includes('script.googleusercontent.com')) && res.status() === 200) {
            try {
                ...
                if(text && text.includes('"rows"') && text.includes('"cells"')) {
                       console.log("INTERCEPTED SPPG: " + url.substring(0, 100));
                       interceptedText = text;
                       fs.writeFileSync(path.join(outputDir, 'data_sppg.json_raw.txt'), text);
                }
            } catch(e) {}
        }
    });
    ...
    if (interceptedText) {
        try {
            ...
            fs.writeFileSync(path.join(outputDir, 'data_sppg.json'), JSON.stringify(data, null, 2));
            console.log(`[SUCCESS] Parsed ${data.length} records into data_sppg.json`);
        } catch(e) {
            console.error("Parse failed");
        }
    } else {
        console.log("Failed to intercept data_sppg payload");
    }
}
