...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const stats = [
    ...
    { label: profile?.role === 'Super Admin' ? 'Total Dana Sewa' : (profile?.role === 'PIC Dapur' ? 'Status BEP' : 'Progres Pembangunan'), 
    ...
    { label: 'Arus Kas', value: `Rp ${(summary?.cash_flow || 0).toLocaleString()}`, change: '+10%', trend: 'up', icon: DollarSign, color: 'emerald' }
  ];

  const quickActions = [
    { label: 'Kelola Dapur', icon: LayoutGrid, action: 'locations', roles: ['Super Admin', 'Manager', 'PIC Dapur'] },
    { label: 'Input Progres', icon: Plus, action: 'construction', roles: ['Super Admin', 'Manager', 'Staff'] },
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-bold text-[#164E4D] tracking-tight">Selamat Datang, {profile?.full_name?.split(' ')[0]}!</h1>
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-2 -mb-2 scrollbar-hide no-scrollbar lg:pb-0 lg:mb-0 lg:overflow-visible">
          {quickActions.map((action) => (
            <button
              ...
              className="flex items-center gap-2 bg-white px-5 py-2.5 rounded-xl text-sm font-semibold text-[#164E4D] border border-gray-100 hover:border-[#1E8289] hover:text-[#1E8289] transition-all shadow-sm active:scale-95 whitespace-nowrap flex-shrink-0"
            >
      ...
      <div className="flex items-center gap-1 bg-[#164E4D]/5 p-1 rounded-2xl w-fit">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${isActive ? 'bg-white text-[#1E8289] shadow-sm' : 'text-gray-500 hover:text-[#164E4D] hover:bg-white/50'}`}
            >
        ...
        <>
              ...
              return (
                 <div key={stat.label} className="glass-card p-6 group hover:shadow-xl hover:-translate-y-1">
                  <div className="flex items-start justify-between mb-6">
                    <div className={`p-4 rounded-2xl ${stat.color === 'mint' ? 'bg-[#F0F7F7] text-[#1E8289]' : 'bg-[#164E4D]/5 text-[#164E4D]'}`}>
                  ...
                  <div>
                    ...
                    <h3 className="text-2xl font-black text-[#164E4D] tracking-tight">{stat.value}</h3>
                  </div>
                </div>
              );
            })}
          ...
          <div className="glass-card overflow-hidden">
            <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-white/30">
              <div>
                <h2 className="text-xl font-bold text-[#164E4D] flex items-center gap-2"><MapPin className="w-5 h-5 text-[#1E8289]" /> Sebaran Dapur MBG</h2>
                <p className="text-sm text-gray-500 font-medium tracking-tight">Monitoring lokasi operasional real-time</p>
              </div>
              <button onClick={() => onNavigate?.('locations')} className="text-xs font-black text-[#1E8289] uppercase tracking-widest hover:underline">Detail Peta</button>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="glass-card p-8 group overflow-hidden">
              <h2 className="text-xl font-bold text-[#164E4D] flex items-center gap-2 mb-8"><TrendingUp className="w-5 h-5 text-[#1E8289]" /> Performa Keuangan</h2>
              <div className="h-64 flex items-end gap-3 px-2">
                {[65, 45, 80, 55, 90, 70, 85].map((val, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2 group/bar">
                    <div className="w-full relative h-[200px] flex items-end">
                       <div className="w-full bg-[#164E4D]/5 rounded-t-lg transition-all duration-500 group-hover/bar:bg-[#1E8289]/10" style={{ height: '100%' }}></div>
                       <div className="absolute bottom-0 w-full bg-gradient-to-t from-[#1E8289] to-[#1E8289]/70 rounded-t-lg shadow-lg shadow-[#1E8289]/20 transition-all duration-700 group-hover/bar:brightness-110" style={{ height: `${val}%` }}></div>
                    </div>
            ...
            <div className="glass-card p-8 overflow-hidden relative">
              <h2 className="text-xl font-bold text-[#164E4D] flex items-center gap-2 mb-8"><Activity className="w-5 h-5 text-[#DE9F22]" /> Progress Pembangunan</h2>
              <div className="space-y-6">
                <div className="relative pt-1">
                  <div className="flex mb-2 items-center justify-between">
                    <span className="text-[10px] font-black py-1 px-2 uppercase rounded-full text-[#DE9F22] bg-[#DE9F22]/10">In Progress</span>
                    <span className="text-sm font-black text-[#164E4D]">{Math.round(summary?.construction_progress || 0)}%</span>
                  </div>
                  <div className="overflow-hidden h-3 mb-4 flex rounded-full bg-[#164E4D]/5 p-0.5 border border-white">
                    <div style={{ width: `${summary?.construction_progress || 0}%` }} className="shadow-lg shadow-[#DE9F22]/20 bg-gradient-to-r from-[#1E8289] to-[#DE9F22] rounded-full transition-all duration-1000"></div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                   <div className="p-4 rounded-2xl bg-[#F0F7F7]/50 border border-white/50">
                      <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Dapur Selesai</p>
                      <p className="text-2xl font-black text-[#164E4D]">{Math.floor((summary?.construction_progress || 0) / 10)}</p>
                   </div>
                   <div className="p-4 rounded-2xl bg-[#F8FAF9] border border-white/50">
                      ...
                      <p className="text-2xl font-black text-[#164E4D]">50</p>
                   </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
               <div className="glass-card overflow-hidden">
                <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-white/30">
                  <h2 className="text-xl font-bold text-[#164E4D] flex items-center gap-2"><Activity className="w-5 h-5 text-[#1E8289]" /> Aktivitas Terbaru</h2>
                  <button onClick={() => onNavigate?.('construction')} className="text-xs font-black text-[#1E8289] hover:underline uppercase tracking-widest">Lihat Semua</button>
                </div>
                <div className="p-8 space-y-8">
                  {activities.map((activity, idx) => (
                    <div key={activity.id || idx} className="flex gap-6 relative group">
                      ...
                      <div className="w-12 h-12 bg-[#F8FAF9] rounded-2xl flex items-center justify-center border border-gray-100 group-hover:border-[#1E8289] transition-colors shadow-sm">
                        <Clock className="w-5 h-5 text-gray-400 group-hover:text-[#1E8289]" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className="font-bold text-[#164E4D] text-base">{activity.title || 'Pembaruan Progress'}</h4>
                        ...
                        <span className="px-3 py-1 bg-[#F0F7F7] text-[#1E8289] rounded-full text-[10px] font-black uppercase">{activity.source || 'Operational'}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-8">
              <div className="glass-card p-8 bg-gradient-to-br from-[#164E4D] to-[#1E8289] text-white overflow-hidden relative shadow-2xl shadow-[#1E8289]/20">
              ...
              <div className="glass-card p-8">
                <h2 className="text-lg font-bold text-[#164E4D] mb-6">Informasi Sistem</h2>
                <div className="space-y-6">
      ...
      {activeTab === 'finance' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="glass-card p-8 group">
             <h2 className="text-xl font-bold text-[#164E4D] mb-4">Ringkasan Keuangan</h2>
             <p className="text-sm text-gray-500 mb-6">Monitoring dana BGN dan alur bagi hasil investor.</p>
             <div className="space-y-4">
                <div className="flex justify-between p-4 bg-[#F0F7F7]/50 rounded-xl">
                  <span className="text-sm font-bold text-gray-600">Total Transaksi</span>
                  <span className="text-lg font-black text-[#164E4D]">Rp {(summary?.cash_flow || 0).toLocaleString()}</span>
                </div>
             </div>
          </div>
          <div className="glass-card p-8 bg-white overflow-hidden relative">
             <h2 className="text-xl font-bold text-[#164E4D] mb-4">Target BEP Investor</h2>
      ...
      {activeTab === 'construction' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           <div className="glass-card p-6 md:col-span-2">
              <h2 className="text-xl font-bold text-[#164E4D] mb-4">Kesehatan Operasional Dapur</h2>
      ...
      {activeTab === 'logistics' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <div className="glass-card p-8">
              <h2 className="text-xl font-bold text-[#164E4D] mb-4">Monitoring Logistik</h2>
              <div className="grid grid-cols-2 gap-6 mt-6">
                 <div className="p-4 bg-gray-50 rounded-2xl">
                    <p className="text-[10px] font-black text-gray-400 uppercase mb-1">Rute Aktif</p>
                    <p className="text-2xl font-black text-[#164E4D]">{kitchens.reduce((acc, k) => acc + (k.routes?.length || 0), 0)}</p>
                 </div>
              </div>
           </div>
        </div>
