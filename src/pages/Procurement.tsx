...
export const Procurement: React.FC = () => {
  ...
  const filteredEquipment = (equipment || []).filter(item => {
    ...
    // In many cases equipment is shared, but if we want to filter by kitchen, 
    // we would need a junction table. For now, we'll assume equipment is global 
    // but future-proof with profile check if the user is a PIC.
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Pengadaan & Inventaris</h1>
          <p className="text-gray-600 mt-1">Kelola pengadaan peralatan dan inventaris dapur MBG</p>
        </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <span className={`px-2 py-1 rounded text-[10px] font-medium ${getStockStatusBadge(item.status || 'in_stock')}`}>
                        {(item.status || 'in_stock') === 'in_stock' ? 'Tersedia' : 
                         (item.status || 'in_stock') === 'low_stock' ? 'Stok Menipis' : 'Stok Habis'}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 font-semibold text-gray-900">Total Biaya</th>
                ...
                <tbody>
                  {orders.filter(o => !profile?.kitchen_id || (o as any).kitchen_id === profile.kitchen_id).map((po) => (
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${getPOStatusBadge(po.status)}`}>
                          {po.status === 'ordered' ? 'Dipesan' : 
                           po.status === 'delivered' ? 'Diterima' : 'Dibatalkan'}
                        </span>
                      </td>
                      <td className="py-3 px-4">
          ...
          {activeTab === 'inventory' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200 text-center">
                ...
                <h3 className="font-bold text-gray-900 mb-2">Manajemen QR Code</h3>
                <p className="text-sm text-gray-600 mb-6">Scan atau cetak QR code baru untuk peralatan dapur</p>
                <button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">Buat Label QR</button>
              </div>
      ...
      {isEquipmentModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-lg w-full p-4">
            <h2 className="text-lg font-bold mb-4">{editingEquipment ? 'Edit Peralatan' : 'Tambah Peralatan Baru'}</h2>
            <form onSubmit={async (e: any) => {
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Nama Peralatan</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Unit Stok</label>
                    <input name="stock" type="number" defaultValue={editingEquipment?.stock} required className="mt-1 w-full border rounded-lg p-2" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Status Stok</label>
      ...
      {isPOModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-lg w-full p-4">
            <h2 className="text-lg font-bold mb-4">{editingPO ? 'Edit Pesanan Pembelian' : 'Buat Pesanan Pembelian'}</h2>
            <form onSubmit={async (e: any) => {
              e.preventDefault();
              const formData = new FormData(e.target);
              const data = {
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Total Biaya (IDR)</label>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">{editingPO ? 'Update PO' : 'Buat PO'}</button>
              </div>
            </form>
          </div>
        </div>
